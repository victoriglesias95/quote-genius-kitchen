...
const EditSupplier = () => {
  ...
  const productForm = useForm<z.infer<typeof productsSchema>>({
  ...
  const handleAddSelectedProducts = () => {
    // Don't add if none selected
    if (selectedProductIds.length === 0) {
      toast.error('Please select at least one product');
      return;
    }
    
    const defaultPrice = productForm.getValues('defaultPrice');
    const newProducts: Product[] = [];
    
    // Process each selected product
    selectedProductIds.forEach(productId => {
      const selectedProduct = productDatabase.find(p => p.id === productId);
      ...
      if (selectedProduct) {
        ...
        if (!existingProduct) {
          ...
          const newProduct: Product = {
            id: `${supplier?.id || 'new'}-${Date.now()}-${selectedProduct.id}`, 
    ...
    if (newProducts.length > 0) {
      ...
      toast.info('All selected products are already in the list');
    }
          ...
          <div className="p-6 md:p-8">
                      ...
                      {products.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan={5} className="p-4 text-center text-gray-500">
                            No products available. Click "Add Products" to add one or more.
                          </td>
                        </tr>
                      )}
                    </tbody>
            ...
            <Dialog open={isAddProductDialogOpen} onOpenChange={setIsAddProductDialogOpen}>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>Add Products from Database</DialogTitle>
                  <DialogDescription>
                    Select one or more products to add to this supplier's list.
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 py-4">
                      ...
                      <div className="divide-y">
                        {filteredProducts.map(product => {
                          ...
                          return (
                            <div 
                              key={product.id}
                              className={`p-2 hover:bg-muted flex justify-between items-center ${isAlreadyAdded ? 'opacity-50' : ''}`}
                            >
                              <div>
                  ...
                  {/* Selected count and price field */}
                    ...
                    <Form {...productForm}>
                      <form onSubmit={(e) => {
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Default Price for Selected Products (Optional)</FormLabel>
                              <FormControl>
                                <Input 
                                  type="number" 
                                  step="0.01" 
